...
__about__ = "" \
"===== About ChineseCipher =====\n\n" \
"ChineseCipher allows you to encrypt and decipher ASCII lowercase text \n" \
...
def do_encrypt(text, chinese=False, samurai=False, digits=False):
...
def encrypt(text, chinese=False, samurai=False, digits=False):
    ...
    return do_encrypt(text, chinese=chinese, samurai=samurai, digits=digits)
...
def test():
    ...
    for i in MAP.keys():
        ...
        chinese, samurai, digits = encrypt(txt, chinese=True, samurai=True,
                                           digits=True)
        dec_c = decipher(chinese)
        dec_s = decipher(samurai)
        dec_d = decipher(digits)
        if txt != dec_c:
...
def main():
    ...
    encrypt_parser.add_argument('-c', '--chinese', action="store_true",
                                help="Output chinese version.")
    encrypt_parser.add_argument('-s', '--samurai', action="store_true",
                                help="Output samurai version.")
    encrypt_parser.add_argument('-g', '--digits', action="store_true",
    ...
    if args.command == "encrypt":
        try:
            ...
            out = encrypt(data, chinese=args.chinese, samurai=args.samurai,
                          digits=args.digits)
            if args.ofile:
